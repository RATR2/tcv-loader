...
# Discord's own color: the one thing here someone reaches for when things aren't working, so let it stand out from the two links beside it that only ever ask for something.
...
func _ready() -> void:
	...
	_refresh()
	# whatever tore the last screen down left its reason here, because that screen
	# had nowhere to print it. That takes priority over the standing status text.
	var notice: String = Net.take_pending_notice()
	if not notice.is_empty(): _set_status(notice)
	elif Net.is_online():
...
# Placed by hand off the viewport, not anchors: world.gd hangs the lobby off PrimaryCapsule (a plain Node), so anchor presets never resolve and this Control stays 0x0.
...
# Covers only "nothing answered the socket"; Net itself now handles "reached them, handshake
# never answered" (a build mismatch, not network).
