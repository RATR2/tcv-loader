...
# Shown in the lobby and log so two people can compare builds; bump for any release,
# even without a protocol change, or "what does yours say?" stops being answerable.
const MOD_VERSION: String = "1.1.9"

const PORT: int = 7654
const MAX_PLAYERS: int = 4
# Bump on any wire-protocol-breaking change, not a payload-only addition (a new handshake key,
# say); an unbumped protocol on two builds that behave differently makes "what does yours say?"
# impossible to answer. See the notes above submit_performance and above the join handshake for what's currently gated by it.
const PROTOCOL_VERSION: int = 7
const CHUNK_SIZE: int = 16384
# How far ahead of the slowest listener the sender may get; caps unacknowledged audio so any
# link that's still moving can drain it before ENet's retransmits run out. 64KB is one round trip's worth from LAN to relayed VPN.
const PERF_WINDOW_BYTES: int = 65536
# A listener that's acked nothing this long isn't listening; stop holding the recorder for them and let that one peer skip the clip rather than freeze everybody.
const PERF_ACK_TIMEOUT: float = 45.0
# Nothing heard about this clip yet; the owner is presumably still recording, maybe re-taking.
# Long on purpose: the old flat 60s timeout skipped people who were just taking their time.
const PERFORMANCE_IDLE_TIMEOUT: float = 300.0
# A take that started arriving and then stopped; can be short since every chunk is acknowledged, so silence here really is silence.
const PERFORMANCE_STALL_TIMEOUT: float = 45.0
const BARRIER_TIMEOUT: float = 90.0
# How long either end waits on the join handshake: it's just a dictionary and two rpcs, so approaching this means an unreachable peer, not slowness.
const HANDSHAKE_TIMEOUT: float = 10.0

# ENet drops a peer that stops acking for 30s by default. Loading a match scene blocks the main
# thread (studio model, contestant packs, dub video all load there, un-yieldable) and can outlast
# that on a slow disk, so extend well past the barrier's own 90s wait. PEER_TIMEOUT_LIMIT (the
# retransmit count) has to grow too, not just MIN/MAX, or the backoff maxes out around 30s regardless of the wider window.
...
# Peer -> voice pack summary, kept out of the roster since the roster resends on every Ready tick, and this only changes on join/leave.
...
# Bumped every show; round state is keyed by round number and slot, both of which restart at
# zero, so without a stamp a finished match's leftovers look identical to the new one.
...
func join_game(address: String, player_name: String, pack: String, port: int = PORT) -> Error:
	# Before the socket, not after: this walks every voice pack on disk, and once connected
	# the only place left to do it is inside the multiplayer poll, stalling the handshake it's holding up.
	voice_pack_manifest()
	var peer: = ENetMultiplayerPeer.new()
...
func end_session() -> void:
	...
	_reset_session_state()
	if is_host():
		# anyone who left mid-show kept their seat so the slots stayed still under
		# it. This is where they lose it. See _on_peer_disconnected.
		_rebuild_slots()
		for id: int in players: players[id]["ready"] = false
		_push_roster()


# a match or a dub is actually running, as opposed to everyone sat in the lobby.
# Cleared with the rest of the session state.
var _show_running: bool = false


var _pending_name: String = ""
var _pending_pack: String = ""

# For whoever opens the lobby next: losing the connection during a show frees the screen that would've shown it, so it waits here for lobby.gd to collect.
...
# Host: peers connected but not yet handshook. Client: whether the host has answered at all,
# different from whether the roster's turned up, and the one worth telling people about.
var _pending_handshakes: Dictionary = {}
var _handshake_answered: bool = false
# Bumped on every dial-out; without this stamp, a watchdog left over from an abandoned attempt closes the connection made on the next try.
...
func _on_peer_disconnected(id: int) -> void:
	...
	# A peer that opened a socket and left without ever registering. The watchdog below only
	# caught this when its own 10s clock ran out first; a joiner giving up early takes the socket
	# with it and erases the entry the watchdog was waiting on. Say it here too.
	...
	if never_handshook:
		...
		host_notice.emit("Someone reached you and left again without getting through the "
			...
			+ _this_build())
	if is_host():
		# Everything below is about a seat coming free, so skip anyone who never made the roster.
		var was_seated: bool = players.has(id)
		players.erase(id)
		manifests.erase(id)
		if was_seated:
			# Slots are what everything mid-show is keyed to (clip ownership, barriers, takes);
			# reshuffling now would point those at the wrong person, so the leaver keeps their
			# seat until end_session() rebuilds. They're still gone from `players`, which is what everyone checks to skip them.
			if _show_running:
				log_net("peer %d left mid-show, holding their seat until the show ends" % id)
			else:
				_rebuild_slots()
				# Roles are keyed by slot, and the leaver's slot just shifted, so keeping them would
				# hand someone else's characters over. Only matters while casting is still being
				# picked; once the dub runs, owners are already resolved and sent.
...
func _on_connected_to_server() -> void:
	...
	_handshake_answered = false
	# Manifest already computed in join_game; walking packs_voice here would stall this poll,
	# and on a big library alone could outlast the host's patience.
	# "mod" is only ever quoted back in a message; new payload keys are free (it's always one
	# Dictionary, see the note by _HANDSHAKE) and a too-old build is told apart by the key's absence.
	_HANDSHAKE.rpc_id(1, {
...
func _on_server_disconnected() -> void:
	var mid_show: bool = _show_running
	log_net("host closed the connection, returning to the lobby")
	mode = MODE.OFFLINE
	players.clear()
	manifests.clear()
	slot_map = []
	...
	# Being dropped mid-show tears the scene down with nowhere left to put the reason; people
	# were bounced to the main menu with no message. Go back to the lobby instead, which can print it.
	if mid_show:
		pending_notice = ("Lost the connection to the host part way through the show.\n"
			+ "If it went quiet rather than them closing the lobby, the link between you "
			+ "could not carry the recordings fast enough. Hamachi and Radmin relay "
			+ "through a server whenever they cannot connect the two of you directly, and "
			+ "a relayed link is often too slow for this. Check both ends show a green "
			+ "dot rather than a blue or flashing one, and try again.")
	else:
		pending_notice = ("Lost the connection to the host. Either they closed the lobby, "
			+ "or the link between you dropped.")
	M.world.CreateLobby.call_deferred()
...
#
# Godot sends @rpc calls by index into the script's methods sorted alphabetically, not by
# name. Add/rename/remove any @rpc anywhere in this file and everything after it shifts a
# slot, while an old-build peer keeps sending the old index, landing on whatever's now
# there and silently dropped on an argument-count mismatch. The version check itself can't
# live in a normal @rpc for this reason: it wouldn't survive the version difference it's meant to catch.
#
# _HANDSHAKE/_HANDSHAKE_REPLY sort ahead of every _rpc_* ('H' < 'r'), pinning them to call 0/1
# forever, as long as everything else keeps the _rpc_ prefix (_ready() checks and shouts if
# not). One Dictionary payload each, so new fields never move the argument count either.
...
# Tell them why, then close it, but not in the same breath: ENet delivers the refusal and
# disconnect together, and acting on the disconnect first throws away the still-unread reason,
# leaving only a bare "lost the connection" with no explanation.
...
# A peer that opened a socket and said nothing this build understands: an old mod copy calling an rpc that's since moved looks exactly like this, and is by far the usual cause.
...
# Every build-difference message carries this: "one of you is on the wrong build" is useless in a screenshot unless it says which build said it.
func _this_build() -> String:
	return "You are on mod %s, protocol %d." % [MOD_VERSION, PROTOCOL_VERSION]


# What to call the other end's build; one too old to send a version is identified by that absence, worth saying instead of printing empty quotes.
...
# The other side: socket's open, host said nothing back. The lobby's own timer only knows no lobby turned up, which reads as a network problem when it usually isn't.
...
# Clip count + hash per pack, never file names: sending names was hundreds of KB, and ENet
# drops a peer 30s after a reliable packet can't get across, which would kick a joiner
# with an empty lobby in front of them. Audio file names only: hashing captions/icons gives constant false mismatches.
#
# Hash covers pack contents, deliberately not the folder name: two people with the same pack
# often have it under different folder names ('SML - Mushroom Pizza' vs 'sml_-_mushroom_pizza'),
# which would otherwise read as two missing packs and throw the joiner out over clips they have all along.
	...
	for pack: String in DirAccess.get_directories_at(root):
		...
		while not stack.is_empty():
			...
			for f: String in DirAccess.get_files_at(dir_path):
				...
				# Backing tracks, _ignore, and the game's own dub recordings; never performed, so counting them just invented false differences between identical packs.
...
# Matched on contents first, so a pack is only mentioned when the clips really differ; a mismatched folder name alone is no longer worth reporting.
...
# Profile.audio_device_in can hold a since-unplugged mic's name (the setter falls back to
# "Default" but leaves the dead name behind, and the game reassigns it unchecked every turn).
# Offline that's one person; online it's everyone. Check it the way the device dropdown does.
...
# The game crops 0.4s off every take by byte count, and slice() past the end returns nothing,
# so a short capture comes out empty, not short (the waveform still draws fine off the spectrum
# analyser, so this is easy to miss). Keep the uncropped take instead of sending nothing.
...
### sending a take ############################################################
#
# A take is ~1MB of raw PCM per clip. Handing the whole thing to ENet as fast as the frame loop
# allows would offer whatever the link can carry with no pushback: ENet queues, retransmits, and
# after ~30s of getting nowhere over a slow relay link, drops the peer mid-take.
#
# So the sender waits for acks and never gets more than PERF_WINDOW_BYTES ahead of the slowest
# listener, keeping ENet's queue to a drainable window that never exhausts its retransmit budget.
# The take is compressed first (lossless, a third smaller), and the ack-gating also keeps both
# ends in step by itself: the recorder doesn't move to the next clip until everyone has the last one.
...
func _rpc_perf_chunk(sid: int, slot: int, chunk: PackedByteArray) -> void:
	...
	staged["data"] = data
	# The ack the sender's window turns on; per chunk rather than periodic, since it's only a
	# handful of bytes and doubles as the on-screen progress for both ends.
...
# Two clocks, not one: waiting on someone who hasn't started sending means waiting on them to
# finish recording (open-ended), while waiting mid-transfer-gone-quiet means a dead link. A
# single timeout can't tell those apart without either skipping thorough recorders or giving up on slow-but-alive transfers.
...
#
# Everyone reaches the last clip within a frame of each other: the whole dub runs in lockstep,
# nobody moves on until the take in front of them has landed. What's not in lockstep is what
# happens next: dub_finished() scores every clip on a background thread plus a one-second wheel
# spin, taking as long as the machine takes, so a peer that starts watching before another has
# finished scoring paints the video over their still-building results screen.
#
# Pressing Watch doesn't start anything by itself. It asks the host, who waits until every peer
# says it's on its results screen, and only then broadcasts the start. Anyone can press it; it means the same thing whoever does.
...
# long enough for a slow machine to finish scoring, short enough that a
...
# Clip index -> slot, resolved once by the host and sent with dub_begin; recomputing it locally would let a late roles push give two players different answers for the same clip.
...
func _on_dub_should_start(folder: String, clip_paths: PackedStringArray) -> void:
	...
	# A dub pack drags the video in with it, the mod's slowest load by far; the host already
	# threads it in clip_selection_dub, and doing it inline here froze the joiner until the host's ENet gave up.
...
# The joiner has no idea the host pressed anything until its scene changes, and the load in between is the slowest part of joining; say so. Empty string clears the hint.
...
func _abort_to_menu(reason: String) -> void:
	...
	connection_failed.emit(reason)
	leave()
	# the lobby rather than the main menu, for the same reason as above: it is the
	# one screen with somewhere to print why this happened.
	pending_notice = reason
	if get_tree().get_root().has_node("World"): M.world.CreateLobby.call_deferred()
...
# Decoding a clip isn't quick, and ENet only gets polled between frames; doing a whole pack in
# one go would stop us acking anything long enough for the host to drop a gone-quiet peer at
# 30s, freezing the lobby right as the host starts playing. Come up for air after every clip.
...
### the way in #################################################################
#
# F9 alone was never enough: laptop function rows default to media keys without Fn, OBS/Medal
# both want F9 for recording, and some people were pressing it on the stock exe. None of that
# improves by reading the key more carefully, so there's a button on screen now too. F9 still works.

const ENTRY_INSET: Vector2 = Vector2(24, 8)
# Room for the mod menu's "MODS" button (button width + a gap) if that mod is
# also installed; see _place_entry_button().
const MODS_BUTTON_RESERVED_WIDTH: float = 110.0
...
func _place_entry_button() -> void:
	...
	# Bottom-right, same corner as the mod menu's "MODS" button, shifted left to leave it room rather than querying it directly, so this still works fine standalone.
	var view: Vector2 = _entry_layer.get_viewport().get_visible_rect().size
	_entry_button.position = Vector2(
		view.x - _entry_button.size.x - ENTRY_INSET.x - MODS_BUTTON_RESERVED_WIDTH,
		view.y - _entry_button.size.y - ENTRY_INSET.y)


# Main menu only: CreateLobby frees everything under primary_capsule, so F9 mid-show would tear the show down for you and everyone waiting on your barrier. Also stops a second press stacking another lobby. menu_master hosts every menu screen as one child of its SlideCapsule, so checking for menu_master alone would allow this from customize/settings/etc. too, not just the home screen.
func can_open_lobby() -> bool:
	...
	for child: Node in capsule.get_children():
		if not child.scene_file_path.contains("menu_master"): continue
		var slide_capsule: Node = child.get_node_or_null("SlideCapsule")
		if slide_capsule == null: return false
		for slide: Node in slide_capsule.get_children():
			if slide.scene_file_path.contains("menu_home"): return true
		return false
	return false
...
# ahead of any focused control that would otherwise swallow the key.
...
func _ready() -> void:
	# CONNECT_DEFERRED for the same reason the lobby uses it: these fire inside the multiplayer
	# poll, and loading a pack/swapping scenes from in there would strand joiners on the lobby screen while the host played on.
...
# The handshake survives version differences only because it sorts to the front of the rpc list,
# and nothing in the language enforces that: an rpc named e.g. _abort_round would quietly steal call 0. Catch it here rather than in someone's lobby.
...
func _on_match_should_start(clip_paths: PackedStringArray) -> void:
	...
	if not missing.is_empty():
		...
		_abort_to_menu("Cannot start: you are missing %d clip(s) the host is using (%s). Copy the host's packs_voice folder across." % [
